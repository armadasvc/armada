...
def encode_and_strip(obj):
    if isinstance(obj, str):
        return obj.strip()
    elif isinstance(obj, list):
        return [encode_and_strip(item) for item in obj]
    elif isinstance(obj, dict):
        return {key: encode_and_strip(value) for key, value in obj.items()}
    else:
        return obj
...
class ArkoseBrowserFingerprint(object):
    def __init__(self, encoded_fingerprint: str, user_agent: str, timestamp: int = int(time.time())):
        ...
        self._raw_fingeprint_2 = encode_and_strip(self._raw_fingeprint)
        self.fingerprint: list[dict[str, Any]] = json.loads(self.crypto.decrypt(self._raw_fingeprint).decode())
       
    def update_timestamp_and_ua(self, new_user_agent,timestamp: int = int(time.time())) -> int:
        self._timestamp = int(timestamp - (timestamp % 21600))
