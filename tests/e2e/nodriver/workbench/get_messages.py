...
def local_get_messages():
    ...
    def replace_env_values(data, env_dict):
        if isinstance(data, dict):
            ...
            env_key = data[5:]
            return env_dict.get(env_key, data)
    ...
    with open("config/data_agent.csv") as file_data_agent:
        reader_data_agent = csv.DictReader(file_data_agent)
        try:
            dict_list_data_agent = [{k: parse_value(v) for k, v in row.items()} for row in reader_data_agent]
        except:
            pass

    with open("config/data_job.csv") as file_data_job:
        reader_data_job = csv.DictReader(file_data_job)
        try:
            dict_list_data_job = [{k: parse_value(v) for k, v in row.items()} for row in reader_data_job]
        except:
            pass
    try:
